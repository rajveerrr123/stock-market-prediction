...
export function AdvancedStockChart({
  ...
  useEffect(() => {
    ...
    const allValues = [
      ...
      ...data.trainingData.filter((v): v is number => v !== null),
      ...data.testingData.filter((v): v is number => v !== null),
    ]
    ...
    // Draw training data (if LSTM model)
    if (modelType === "lstm" && data.trainingData.length > 0) {
      ...
      for (let i = 0; i < data.trainingData.length; i++) {
        const y = getY(data.trainingData[i])
    ...
    // Draw testing data (if LSTM model)
    if (modelType === "lstm" && data.testingData.length > 0) {
      ctx.strokeStyle = "#f59e0b" // amber-500
      ctx.lineWidth = 2
      ctx.beginPath()

      let testingStarted = false
      for (let i = 0; i < data.testingData.length; i++) {
        const y = getY(data.testingData[i])

        if (y !== null) {
          const x = getX(i + data.trainingData.filter((d) => d !== null).length)
    ...
    if (modelType === "lstm") {
      ...
      ctx.fillStyle = "#fff"
      ctx.fillText("Training Data", legendX + 25, legendY + 64)
      ...
      ctx.lineTo(legendX + 20, legendY + 80)
      ctx.stroke()

      ctx.fillStyle = "#fff"
      ctx.fillText("Testing Data", legendX + 25, legendY + 84)
    }
  }, [data, modelType])
